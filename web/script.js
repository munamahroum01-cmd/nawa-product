document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkMobile() {
        ...
        if (container) {
            ...
            if (isMobile) {
                // STABLE MOBILE ROTATION: Rotate the BOOK, not the container
                const scale = (window.innerWidth * 0.94) / bookHeight;
                book.style.transform = `rotate(90deg) scale(${scale})`;
                container.style.transform = 'none'; // Clear from container
            } else {
                container.style.transform = `translateX(${bookWidth / 2}px)`;
